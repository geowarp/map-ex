...
export function TimeSliderPanel({
  ...
  return (
    <div className="w-125 bg-white/40 backdrop-blur-md border border-white/20 shadow-lg rounded-lg p-3 flex items-center gap-3">
      {/* Previous Button */}
      <button
        onClick={handlePrevious}
        disabled={currentIndex === 0}
        className="shrink-0 w-8 h-8 rounded-md bg-black/10 hover:bg-black/20 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center transition-colors"
      ...
      <div className="flex-1">
        <Slider
          ...
          max={maxIndex}
          step={1}
          className="**:data-[slot=slider-track]:bg-black/20 **:data-[slot=slider-range]:bg-black/30 **:data-[slot=slider-thumb]:bg-orange-500 **:data-[slot=slider-thumb]:border-orange-500 **:data-[slot=slider-thumb]:w-4 **:data-[slot=slider-thumb]:h-4"
        />
      </div>
      ...
      {/* Date Time Display */}
      <div className="shrink-0 text-xs font-medium text-foreground whitespace-nowrap min-w-25 text-right">
