...
export function WeatherLayersPanel({ onLayerChange, selectedLayer: externalSelectedLayer, isDisabled = false, disabledMessage }: WeatherLayersPanelProps) {
  const [selectedLayer, setSelectedLayer] = useState<WeatherLayerType>("temperature")
  const [loading, setLoading] = useState(false)

  const layers: LayerOption[] = [
    { id: "temperature", label: "Temperature", icon: <Thermometer className="h-5 w-5" /> },
    { id: "pressure", label: "Pressure", icon: <Gauge className="h-5 w-5" /> },
    { id: "wind_arrows", label: "Wind", icon: <Wind className="h-5 w-5" /> },
    { id: "precipitation", label: "Precipitation", icon: <Droplet className="h-5 w-5" /> },
  ...
  useEffect(() => {
    ...
    loadLayerConfiguration(currentSelectedLayer)
  }, [])
  ...
  return (
    <Card className={`w-[220px] bg-white/40 backdrop-blur-md border-white/20 shadow-lg p-3 ${isDisabled ? 'opacity-60' : ''}`}>
      ...
      <div className="flex flex-col gap-1">
        {layers.map((layer) => {
          ...
          return (
            ...
            >
              <span className={isSelected ? "text-white" : "text-foreground/50"}>
                {layer.icon}
              </span>
